...
from src.schemas import (
    ...
    TeamProfileSeason,
    TeamProfileKPIs,
    TeamProfileGameResult,
    TeamProfileTopScorer,
    FormResult,
)
...
async def team_profile(
    ...
    season: int | None = Query(
        None, description="Season year; defaults to most recent"
    ),
    ...
    seasons_result = await db.execute(
        select(distinct(Game.season_year))
        .where(
    ...
    conf_avg_gpg = (
        round(sum(conf_total_gpg) / len(conf_total_gpg), 2) if conf_total_gpg else 0.0
    )
    conf_avg_conv = (
        round(sum(conf_total_conv) / len(conf_total_conv), 1)
        if conf_total_conv
        else 0.0
    )
    conf_avg_gapg = (
        round(sum(conf_total_gapg) / len(conf_total_gapg), 2)
        if conf_total_gapg
        else 0.0
    )
