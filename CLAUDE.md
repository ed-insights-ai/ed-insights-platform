...
(The figures above were re-measured against Postgres on 2026-08-09 and all four match
exactly. The much older set — 18 schools / 2,852 games / 103,621 player stats / 27,238
events / 5,704 team stats — predates migration 005 and should never be quoted.)

**The retired-ordinal problem in `data/structured/all/` is GONE** — that export was
regenerated by PR #52 and now carries 2,140 games with **0** rows from retired ordinals,
matching Postgres. The `export-staleness` check added in PR #54 verifies this and currently
reads *ok*.

One parquet discrepancy remains and it is real: the **per-season** event parquets hold
**22,782** events while both the merged export and Postgres hold **22,754**. Those 28 events
— one of them a goal — are destroyed by an event-id collision. The parser fix landed in PR
#54; the regeneration and reload that would actually recover them is deferred on `tl-3zm`.
Until that runs, prefer Postgres for game/player/team counts, but know that it is **28
events short of the truth**.
