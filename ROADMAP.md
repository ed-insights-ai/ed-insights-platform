...
**0/52 tasks complete** across 12 epics · **14 ready to start** · 38 waiting on a blocker
...
| P0 | `tl-bbu` | Migration 005 deletes all data in both directions, and runs on every start (gh-19) ([#19](https://github.com/ed-insights-ai/ed-insights-platform/issues/19)) | S0 |
...
| P1 | `tl-3zm` | Purge 712 retired-ordinal rows from exported parquets (gh-27) ([#27](https://github.com/ed-insights-ai/ed-insights-platform/issues/27)) | S0.5 |
...
| P1 | `tl-oyr` | Scrape reports success on partial and rejected runs (gh-17) ([#17](https://github.com/ed-insights-ai/ed-insights-platform/issues/17)) | S0 |
...
- [ ] `tl-oyr` **P1** Scrape reports success on partial and rejected runs (gh-17) ([#17](https://github.com/ed-insights-ai/ed-insights-platform/issues/17))
...
- [ ] `tl-4jg` **P0** Verify the repair — before/after for every named defect — *blocked by `tl-9ap`, `tl-09k`, `tl-o23`, `tl-vkt`, `tl-hbo`, `tl-37d`, `tl-bbu`, `tl-qbg`, `tl-dse`, `tl-4ix`*
...
- [ ] `tl-3zm` **P1** Purge 712 retired-ordinal rows from exported parquets (gh-27) ([#27](https://github.com/ed-insights-ai/ed-insights-platform/issues/27))
...
New repo: **github.com/ed-insights-ai/keelson-rib-touchline**. Package skeleton copied verbatim from keelson-rib-workiq.
...
- [ ] `tl-eus` **P1** touchline-refresh in observe-only form — *blocked by `tl-kla`*
- [ ] `tl-tre` **P1** Boot self-check — every cadence-bearing region has a matching bound producer — *blocked by `tl-kla`*
...
- [ ] `tl-a2n` **P0** The complete 7-node touchline-refresh — *blocked by `tl-4nx`, `tl-eus`, `tl-oyr`*
...
- [ ] `tl-gnu` **P4** The caution Type column at sidearm_parser.py:339-348 — *blocked by `tl-9ap`*
